...
export function usePaymentSetup(options: UsePaymentSetupOptions): UsePaymentSetupResult {
  ...
  const setupPayment = useCallback(async (stripe: any, elements: any) => {
    ...
    try {
      ...
      if (oldPaidPaymentMethodId) {
        requestBody.oldPaidPaymentMethodId = oldPaidPaymentMethodId;
      }
  ...
  }, [customerID, provider, apiEndpoint, oldPaidPaymentMethodId, onSuccess, onError]);
